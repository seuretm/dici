...
class ProjView(QMainWindow):
    ...
    def initUI(self, DataDisplayClass):
        self.setWindowTitle('FAU - Lange Nacht der Wissenschaften')
        wid = QWidget(self)
        ...
        customMenu = self.data_display.custom_menu()
        if customMenu:
            if not isinstance(customMenu, (tuple, list)):
                customMenu = (customMenu,)
            dataMenu = menu.addMenu('Data')
            for btn in customMenu:
                dataMenu.addAction(btn)
        ...
        btn = QAction('Dirichlet Process Gaussian Mixture Model (slow)', self)
...
class PlotCanvas(FigureCanvas):
    ...
    def __init__(self, parent, width=5, height=4, dpi=100):
        ...
        self.fig.canvas.mpl_connect('scroll_event', self.mouse_zoom)
    ...
    def onclick(self, event):
        ...
        self.axes.scatter(self.data[self.closest,0], self.data[self.closest,1], color='r', s=2, marker='x')
        self.highlighted = self.axes.scatter(self.data[self.closest,0], self.data[self.closest,1], color='r', s=12)
        self.draw()
    
    def mouse_zoom(self, event, base_scale=2.0):
        ...
        if event.button == 'up':
            ...
            scale_factor = 1
            print('Unknown wheel direction:', event.button)
