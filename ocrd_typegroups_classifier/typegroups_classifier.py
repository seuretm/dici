...
class TypegroupsClassifier:
    ...
    def describe(self, pil_image, stride, batch_size=16):
        ...
        with torch.no_grad():
            ...
            for x in range(0, pil_image.size[0], stride):
                for y in range(0, pil_image.size[1], stride):
                    ...
                    if len(batch) >= batch_size:
                        tensors = torch.stack(batch).to(self.dev)
                        f, c = self.network.describe(tensors)
                        features += f.sum(0)
                        scores   += c.sum(0)
                        processed_samples += len(batch)
                        batch = []
        ...
        if was_training:
            self.network.train()
        features /= processed_samples
        scores   /= processed_samples
        return (features.cpu().numpy(), scores.cpu().numpy())
